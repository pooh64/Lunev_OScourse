		 ...
		 int (*self_init) (int))
{
	...
	/* User-defined init */
	if (self_init != NULL && (*self_init) (semid) == -1) {	/* Enter critical section 2 */
...
int pair_release(int semid, struct syncbuf self, struct syncbuf othr)
{
	...
	SOPBUF_ADD(othr.done,  1, 0);
	SOPBUF_ADD(self.lock, -1, SEM_UNDO);
	if (SOPBUF_SEMOP() == -1) {		/* Barier */	/* Leave critical section 1 */
		if (errno == EAGAIN)
	...
	SOPBUF_ADD(self.actv, -1, SEM_UNDO);
	if (SOPBUF_SEMOP() == -1) {		/* Barier */ 	/* Leave critical section 2 */
		perror("Error: semop");
		return -1;
	}
